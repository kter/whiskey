...
const selectForReview = (result: WhiskeySuggestion) => navigateTo({
  path: '/reviews/new',
  query: {
    whiskey_id: result.id,
    whiskey_name: result.name_ja || result.name_en || result.name,
...
<template>
  <div class="max-w-5xl mx-auto">
    ...
    <p v-else-if="advancedResults.length === 0 && !isAdvancedSearching" class="text-center py-12 text-amber-400">該当するウイスキーが見つかりませんでした。</p>
    <div v-else class="mt-6 space-y-4">
      <article v-for="result in advancedResults" :key="result.id" class="bg-stone-800 p-6 rounded-lg border border-amber-700">
        <div class="flex justify-between gap-4"><div><h2 class="text-lg font-medium text-amber-200">{{ result.name_ja || result.name_en || result.name }}</h2><p v-if="result.distillery" class="text-amber-300">{{ result.distillery }}</p><p class="text-sm text-amber-400">{{ [result.region, result.type].filter(Boolean).join(' • ') }}</p></div><div class="flex flex-col gap-2"><button class="px-3 py-1.5 rounded-md text-amber-100 bg-amber-800" @click="selectForReview(result)">レビュー作成</button><button class="px-3 py-1.5 rounded-md text-amber-200 bg-stone-700" @click="selectedResult = result">詳細</button></div></div>
      </article>
      <div v-if="advancedNextToken" class="text-center"><button :disabled="isAdvancedSearching" class="px-5 py-2 rounded-md text-amber-100 bg-amber-800 disabled:opacity-50" @click="loadMoreAdvancedResults()">{{ isAdvancedSearching ? '読み込み中...' : 'さらに読み込む' }}</button></div>
    </div>

    <div v-if="selectedResult" class="fixed inset-0 z-50 flex items-center justify-center bg-black/75 px-4" @click="selectedResult = null"><div class="max-w-lg w-full bg-stone-800 rounded-lg p-6 border border-amber-700" @click.stop><h2 class="text-xl text-amber-200">{{ selectedResult.name_ja || selectedResult.name_en || selectedResult.name }}</h2><p class="mt-2 text-amber-100">{{ selectedResult.description || '説明はありません。' }}</p><div class="mt-6 flex justify-end gap-3"><button class="px-4 py-2 bg-stone-700 text-amber-200 rounded-md" @click="selectedResult = null">閉じる</button><button class="px-4 py-2 bg-amber-800 text-amber-100 rounded-md" @click="selectForReview(selectedResult)">レビュー作成</button></div></div></div>
  </div>
</template>
